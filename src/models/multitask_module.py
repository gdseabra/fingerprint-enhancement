...
class EnhancerLitModule(LightningModule):
    ...
    def predict_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        output_paths = { "gabor": os.path.join(self.output_path, "gabor"), "bin": os.path.join(self.output_path, "bin"), "enh": os.path.join(self.output_path, "enh"), "dirmap": os.path.join(self.output_path, "dirmap"), "dirmap_png": os.path.join(self.output_path, "dirmap_png"), "mask": os.path.join(self.output_path, "mask") }
        ...
        for i, name in enumerate(names):
            orig = (255 * (latent_enh[i, 0].cpu().numpy().clip(0, 1))).astype('uint8')
            Image.fromarray(orig).save(os.path.join(output_paths["enh"], f"{name}.png"))
            gabor_sig = torch.sigmoid(latent_enh[i, 1])
            bin_img = (255 * torch.round(gabor_sig).cpu().numpy()).astype('uint8')
            Image.fromarray(bin_img).save(os.path.join(output_paths["bin"], f"{name}.png"))
            self.save_orientation_field(dirmap_pred[i].unsqueeze(0), None, os.path.join(output_paths["dirmap_png"], f"{name}.png"), os.path.join(output_paths["dirmap"], f"{name}.dir"))
    # <-------------------------------------------------------------------------------------->